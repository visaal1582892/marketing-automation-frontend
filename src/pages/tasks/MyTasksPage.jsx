...
export default function MyTasksPage() {
  ...
  const submitForQc = async () => {
    ...
    try {
      ...
      showToast(submitting.parentTaskId ? 'Content submitted to the designer.' : 'Submitted for QC review.', 'success')
      setSubmitting(null)
      setTaskQuestions([])
      setTaskAnswers({})
      refresh()
...
function TaskCard({ task, now, hoursSnapshot, busy, closed, isNextUp, hasInFlight, onAccept, onSubmit, onView, onComment, onWorkerUnhold, onCommentAnswered, onCollaborate, canAddFollowup, onLinkedTask, currentUserId }) {
  ...
  return (
    <div className={`rounded-xl border bg-white shadow-sm overflow-hidden ${
      ...
      {task.parentTaskId && <ReferenceTaskPanel taskId={task.taskId} parentTaskId={task.parentTaskId} />}
...
function LinkedTaskBadge() {
  return (
    <span className="inline-flex items-center rounded-full bg-violet-50 px-2 py-0.5 text-xs font-medium text-violet-700 ring-1 ring-violet-200" title="Linked Task / Sub-task">
      Linked Task
    </span>
  )
}
...
function ReferenceTaskPanel({ taskId, parentTaskId }) {
  ...
  useEffect(() => {
    let alive = true
    tasksApi.getParentTask(taskId)
      .then(res => { if (alive) setParent(res.data) })
      .catch(() => {})
